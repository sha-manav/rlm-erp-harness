...
**4. Execute on main, verify, audit, then finish.** Run `plan_fn(erp)`, run
`check.all()`, then **delegate one independent audit** of the main database against the
instruction before you finish: `delegate("Audit the main database against these
requirements: <the instruction's rules, verbatim>. Check every order, purchase, MO,
receipt date, origin and invoice created today; report each violation with the record
and the fix, or say it is clean.")`. Act on what it reports, then call `finish(summary)`.
`finish` refuses until an audit has run, and while a hard check fails — that refusal is
the harness telling you what is still wrong, so read it and fix it rather than retrying.
